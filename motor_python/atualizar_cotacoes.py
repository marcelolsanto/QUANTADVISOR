...
def baixar_e_injetar(ticker):
    print(f"📥 Baixando 10 anos de histórico de {ticker}...")
    try:
        tem_numero = any(char.isdigit() for char in ticker)
        ticker_yf = f"{ticker}.SA" if tem_numero and not ticker.endswith('.SA') else ticker
        
        df = yf.download(ticker_yf, period="max", interval="1d", progress=False, threads=False)
        
        if isinstance(df.columns, pd.MultiIndex):
            coluna_fechamento = df['Close'][ticker_yf]
            coluna_volume = df['Volume'][ticker_yf]
        else:
            coluna_fechamento = df['Close']
            coluna_volume = df['Volume']
            
        coluna_limpa = coluna_fechamento.ffill().dropna()
        col_vol_limpa = coluna_volume.ffill().dropna()
        
        if len(coluna_limpa) < 100:
            print(f"⚠️ {ticker} ignorado. Apenas {len(coluna_limpa)} dias encontrados.")
            return False

        historico_lista = coluna_limpa.values.flatten().tolist()
        volume_lista = col_vol_limpa.values.flatten().tolist()
        
        rdb.set(f"hist:{ticker}", json.dumps(historico_lista))
        # 🚀 NOVO: Salva o histórico de volume em uma chave paralela
        rdb.set(f"vol:{ticker}", json.dumps(volume_lista)) 
        
        print(f"✅ Sucesso: {ticker.ljust(6)} -> {len(historico_lista)} pregões mapeados na RAM!")
        return True
...
def rodar():
    print(f"🚀 Iniciando a Máquina do Tempo para {len(ativos)} ativos...")
    with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
        resultados = list(executor.map(baixar_e_injetar, ativos))
        
    sucessos = sum(1 for r in resultados if r)
    print(f"\n🏁 Operação concluída! {sucessos} ativos prontos para treinar as IAs.")
