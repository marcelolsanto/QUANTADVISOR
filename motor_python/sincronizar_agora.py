...
# A lista de ativos que estavam com preços "fantasmas" no seu print, 
# mais alguns gigantes para garantir a limpeza do cache.
ativos = [
...
print(f"🚀 Iniciando Sincronização Cirúrgica de {len(ativos)} ativos...\n")

sucessos = 0
erros = 0

for i, ticker in enumerate(ativos):
    # 🎯 JÁ CONFIGURADO PARA USAR A BRAPI EM VEZ DO YAHOO
    url = f"http://coletor_go:8080/streaming/ingestao/{ticker}/YAHOO?selic=0.1450"
    #url = f"http://127.0.0.1:8000/streaming/ingestao/{ticker}/BRAPI?selic=0.1450"
    ...
    try:
        ...
        if resposta.status_code == 200:
            print(f"[{i+1}/{len(ativos)}] ✅ {ticker.ljust(6)} -> Marcado a mercado via BRAPI!")
            sucessos += 1
        else:
            print(f"[{i+1}/{len(ativos)}] ⚠️ {ticker.ljust(6)} -> Erro: {resposta.text}")
            erros += 1
            
    except requests.exceptions.ConnectionError:
        print(f"❌ Falha ao conectar. O seu servidor FastAPI (porta 8000) está rodando?")
        break

    # 🛡️ TRAVA ANTI-BANIMENTO: 2 Segundos de pausa (Essencial em ambientes Cloud)
    time.sleep(2)

print(f"\n🏁 Sincronização concluída! Sucessos: {sucessos} | Erros: {erros}")
print("Pode dar F5 na página do React. O Dashboard agora está com os preços reais.")